...
BEGIN
	...
	BEGIN TRY
		...
		IF EXISTS(SELECT 1 FROM gastos.Limpieza WHERE IdLimpieza = @IdLimpieza and IdGO = @IdGO)
		BEGIN

			DELETE FROM Externos.Empleado
			WHERE IdLimpieza = @IdLimpieza AND IdGO = @IdGO;

			DELETE FROM Externos.Empresa
			WHERE IdLimpieza = @IdLimpieza AND IdGO = @IdGO;

			DELETE FROM gastos.Limpieza
			WHERE IdLimpieza = @IdLimpieza AND IdGO = @IdGO;
		...
		ELSE
		BEGIN
			PRINT('No existe el registro de Limpieza con ID: ' + CAST(@IdLimpieza AS VARCHAR));
			RAISERROR('No existe el registro de Limpieza', 10, 1);
	...
	BEGIN CATCH
		IF ERROR_SEVERITY() > 10
		BEGIN
			PRINT('Ocurrio un error en SP gastos.sp_BorrarLimpieza: ' + ERROR_MESSAGE());
			IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
			RAISERROR('ERROR en el borrado de limpieza', 16, 1);

		END
		IF ERROR_SEVERITY() = 10
		BEGIN
			PRINT('Advertencia en SP gastos.sp_BorrarLimpieza: ' + ERROR_MESSAGE());
			COMMIT TRANSACTION;
